...
const ItineraryPage = () => {
  ...
  const handleExportPDF = async () => {
    ...
    doc.text(`Destination: ${plan.destination || itinerary.destination || ''}`, 14, 32);
    if (itinerary.startDate) {
      doc.text(`Dates: ${new Date(itinerary.startDate).toLocaleDateString()} — ${new Date(itinerary.endDate).toLocaleDateString()}`, 14, 38);
    }
    doc.text(`Generated by TravelMind AI`, 14, 44);
    ...
    for (const day of days) {
      ...
      doc.setFontSize(14);
      doc.setTextColor(40);
      doc.text(`Day ${day.dayNumber || ''}: ${day.title || ''}`, 14, yPos);
      yPos += 8;
    ...
    if (plan.travelTips?.length) {
      ...
      doc.setFontSize(14);
      doc.setTextColor(40);
      doc.text('Travel Tips', 14, yPos);
      yPos += 8;
      doc.setFontSize(9);
      doc.setTextColor(80);
      for (const tip of plan.travelTips) {
        if (yPos > 275) { doc.addPage(); yPos = 20; }
        doc.text(`[${tip.category}] ${tip.tip}`, 14, yPos, { maxWidth: 180 });
        yPos += 6;
      }
    }
